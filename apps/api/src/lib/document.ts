...
export async function getUserDocuments(userID: string): Promise<DocumentType[]> {
    const docs = await db.select().from(documents).where(eq(documents.owner_id, userID)).orderBy(desc(documents.created_at)).execute()
    return docs as any
}
...
export async function updateDocumentByID(id: string, content: string): Promise<DocumentType | null> {
